...
cardsValue notAceV aceN = 
  if (aceN > 0) 
    then 
      if (notAceV + 11 + aceN - 1 > 21) 
        then notAceV + aceN 
        else notAceV + 11 + aceN - 1 
    else notAceV
...
isBlackjack hand = 
  if (value hand == 21 && length hand == 2) 
    then True 
    else False
...
winner guest bank = 
  if (value guest > value bank && not (gameOver guest)) 
    then Guest 
    else Bank
...
handSuit suit = 
  ...
  [Card r suit | r <- [Jack, Queen, King, Ace]]
...
fullDeck :: Hand
fullDeck = 
  [Card (Numeric n) suit | n <- [2..10], suit <- [Hearts, Spades, Diamonds, Clubs]] ++ 
  [Card r suit | r <- [Jack, Queen, King, Ace], suit <- [Hearts, Spades, Diamonds, Clubs]]
...
instance Ord Date where
    (>) (Date d1 m1 y1) (Date d2 m2 y2) = y1 > y2 
      || (y1 == y2 && m1 > m2) 
      || (y1 == y2 && m1 == m2 && d1 > d2)
    (<) (Date d1 m1 y1) (Date d2 m2 y2) = y1 < y2 
      || (y1 == y2 && m1 < m2) 
...
employeesWithOverlappingPermits emps = 
  concat (employeesWithOverlapping (withPermit emps))

employeesWithOverlapping :: [Employee] -> [[(EId, EId)]]
employeesWithOverlapping [] = []
employeesWithOverlapping emps = 
  ((overlappingPermits emps) : employeesWithOverlapping (tail emps))

overlappingPermits :: [Employee] -> [(EId, EId)]
overlappingPermits [] = []
overlappingPermits (em@(Emp _ joinedOn1 (Just permit1) _):emps) = 
  map (\e -> ((empId em), (empId e))) 
    (filter (\(Emp _ joinedOn (Just permit) _) -> 
      joinedOn1 <= expiryDate permit 
        || joinedOn <= expiryDate permit) emps)

withPermit :: [Employee] -> [Employee]
withPermit [] = []
withPermit emps = 
  filter (\(Emp _ _ permit _) -> 
    permit /= Nothing) emps
...
employeesTupple [] = []
employeesTupple emps = 
  (getTenure (head emps), 
  filter (\emp -> getTenure emp == getTenure (head emps)) emps) 
  : employeesTupple (filter (\emp -> getTenure emp /= getTenure (head emps)) emps)
...
withExpiredPermit [] _ = []
withExpiredPermit emps currentDate = 
  map (\(Emp eId _ _ _) -> eId) 
    (filter (\(Emp _ _ (Just permit) _) -> 
      currentDate > (expiryDate permit)) (withPermit emps))
...
avgYearsWorked [] = 0
avgYearsWorked emps = 
  (fromIntegral (sumTenure (withLeftDate emps))) 
  / fromIntegral (length (withLeftDate emps))
...
indexToColumn 0 = ""
indexToColumn n = 
    indexToColumn ((n - 1) `div` 26) ++ 
    [toEnum (ord 'A' + (n - 1) `mod` 26)]
...
evalCell pos sheet = case lookup pos sheet of
    ...
    Just (Reference ref) -> 
        case parseReference ref >>= (`lookup` sheet) of
            Just (Number x) -> Just x
            Just (Formula f) -> Just (f sheet)
            Just (Reference newRef) -> 
              parseReference newRef >>= \newPos -> 
              evalCell newPos sheet
            _ -> Nothing
    Nothing -> Nothing
...
mapRange f (r1, c1) (r2, c2) sheet =
    [if r1 <= r && r <= r2 && c1 <= c && c <= c2 
      then (pos, apply val) 
      else (pos, val) | (pos@(r, c), val) <- sheet]
